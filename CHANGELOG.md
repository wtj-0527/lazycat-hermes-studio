...
- rootfs 基线快照在 WatchCat 可用时进入设备级 FIFO 队列，同一台 LazyCat 设备只允许一个 Hermes Studio 实例执行高 I/O 快照。
- 租约每 30 秒续期并在完成、退出或收到终止信号时释放；WatchCat 重启后仍保留有效租约，避免恢复期间并发进入。
- `rm` 与 `cp` 使用 idle I/O 调度和较低 CPU 优先级，降低机械盘繁忙时对系统交互的影响。
- 删除复制期间每 3 秒递归执行 `du` 的进度扫描，避免 Btrfs 元数据被重复遍历。
- 未安装 WatchCat 的设备保持兼容：跳过队列协调，但仍启用低 I/O 优先级。
