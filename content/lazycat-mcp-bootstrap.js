...
export async function syncManagedMcp(fetchImpl = fetch) {
  ...
  const desired = new Map(providers.map(provider => [managedName(provider), {
    url: `http://nginx${provider.proxy_path}`,
    _lazycat_managed: true,
  }]))

  let changed = false
  for (const [name, config] of desired) {
    const existing = servers.get(name)
    if (existing && existing.raw_config?._lazycat_managed !== true) continue
  ...
  for (const server of servers.values()) {
    if (server.name.startsWith(PREFIX) && server.raw_config?._lazycat_managed === true && !desired.has(server.name)) {
